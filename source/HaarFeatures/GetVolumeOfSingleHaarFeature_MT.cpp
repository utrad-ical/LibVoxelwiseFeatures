...
void* threaded_GetVolumeOfSingleHaarFeatureInRoiBoundingBox(void* args)
{
	THREADING_HAARFEATUREVOLUME* a = (THREADING_HAARFEATUREVOLUME*)args;
	float*** featvol = (float***)a->output->array4D[0];
	CalculateSingleHaarFeature_forVoxelSubset(a->input, a->ch, a->subset, a->kernel, a->index, featvol);
	return NULL;
}
...
//	}
	...
	for(int t=0; t<numThreads; t++) {
		pthread_create(&threads[t], NULL, threaded_GetVolumeOfSingleHaarFeatureInRoiBoundingBox, (void*)t_args[t]);
	}
